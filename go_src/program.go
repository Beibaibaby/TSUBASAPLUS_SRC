...
func serializeBWRDFT(bwrdft *BasicWindowDFTResult, rowBWRDFT *RowBWRDFT) {
  ...
  //slicesToString(bwrdft.slicesOfCXY, &rowBWRDFT.cXY)
...
func deserializRowBWRDFT(rowBWRDFT *RowBWRDFT, bwrdft *BasicWindowDFTResult, start int, end int) {
  ...
  //stringToSlices(&rowBWRDFT.cXY, bwrdft.slicesOfCXY, start, end)
...
func getBasicWindowResult(dataMap *(map[int][]Point), granularity int,
  ...
  if !isDFT {
    ...
    //bwrdft.slicesOfCXY = &slicesOfCXY
...
func main() {
  ...
  inputArgs := fmt.Sprintf("fileName: %s, before: %d, numOfLocations: %d, thres: %.2f, granularity: %d, writeBlockSize: %d, readBlockSize: %d, ratio: %.2f, queryStart %d, queryEnd %d", 
    fileName, before, numOfLocations, thres, granularity, writeBlockSize, readBlockSize, ratio, queryStart, queryEnd)
  ...
  // TSUBASA DFT with parallel computing, integreted with PostgreSQL
  /*clearMatrix(&matrix)
  clearSliceOfString(&sketchDurations)
  clearSliceOfString(&queryDurations)
  t7 := time.Now()
  ...
  var sketchTime, queryTime float64
  clearMatrix(&matrix)
  t8 := time.Now()
  networkConstructionBWInMemo(&dataMap, &matrix, thres, granularity, false, ratio, &sketchTime, &queryTime)
  elapsed = time.Since(t8)
  checkMatrix(&matrix)
  fmt.Println("Running time: ", elapsed)
  /*var sketchSum, querySum, totalSum float64 = 0, 0, 0
  for i := 0; i < 20; i += 1 {
    var sketchTime, queryTime, totalTime float64
    getNetworkInMemo(&dataMap, &matrix, thres, granularity, true, ratio, &sketchTime, &queryTime, &totalTime)
    sketchSum += sketchTime
    querySum += queryTime
    totalSum += totalTime
  }
  fmt.Println(sketchSum/20)
  fmt.Println(querySum/20)
  fmt.Println(totalSum/20)*/

  // TSUBASA on single node, in-memory
  clearMatrix(&matrix)
  t9 := time.Now()
  networkConstructionBWInMemo(&dataMap, &matrix, thres, granularity, true, ratio, &sketchTime, &queryTime)
  elapsed = time.Since(t9)
  checkMatrix(&matrix)
  fmt.Println("Running time: ", elapsed)
