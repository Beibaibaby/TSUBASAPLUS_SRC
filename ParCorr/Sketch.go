...
// Update dot products (Sketches) for a batch of time series based on statistics incrementally
func updateBatchSketches(allTimeSeries [][]float64, start, end int, oldSRMSum, newSRMSum []int, randomMatrix [][]int8, prevTSProfiles []TimeSeriesProfile, startPos int, tsProfilesChan chan []TimeSeriesProfile) {
	tsProfiles := make([]TimeSeriesProfile, end-start)

	for id := start; id < end; id++ {
		tsProfile := updateSingleSketch(id, allTimeSeries[id], oldSRMSum, newSRMSum, randomMatrix, prevTSProfiles[id], startPos)
		tsProfiles[id-start] = tsProfile
	}

	tsProfilesChan <- tsProfiles
}

// Update dot product (Sketch) for a single time series based on statistics incrementally
func updateSingleSketch(id int, timeSeries []float64, oldSRMSum, newSRMSum []int, randomMatrix [][]int8, prevTSProfile TimeSeriesProfile, startPos int) TimeSeriesProfile {
	...
	return TimeSeriesProfile{id, newMean, newStdev, newSketch, timeSeries[startPos : startPos+windowSize]}
}
