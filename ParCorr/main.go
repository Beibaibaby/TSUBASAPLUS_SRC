...
func main() {
	...
	gridSizePtr := flag.Int("gridSize", 1, "Size of grid cell")
	fractionThrPtr := flag.Float64("fractionThr", 0.5, "Fraction of grids of collocation")
	windowSizePtr := flag.Int("windowSize", 50, "Size of window")
	basicWindowSizePtr := flag.Int("basicWindowSize", 20, "Size of basic window")
	...
	basicWindowSize = *basicWindowSizePtr
	log.Println("gridSize:", gridSize, "fractionThr:", fractionThr, "windowSize", windowSize, "basicWindowSize", basicWindowSize)
	...
	var (
		...
		accuracies             []float64
		parcorrDurations       []time.Duration
		parcorrSketchDurations []time.Duration
		parcorrSearchDurations []time.Duration
		linearSearchDurations  []time.Duration
	)
	...
	parcorrDurations = append(parcorrDurations, duration)
	log.Println("ParCorr Finished. Costs ", duration)

	log.Println("Linear Search Start...")
	start = time.Now()
	gtPairs := parallelSingleLinearSearch(allTimeSeries, 0)
	duration = time.Since(start)
	linearSearchDurations = append(linearSearchDurations, duration)
	...
	for i := 1; i < windowNum; i++ {
		...
		log.Println("ParCorr Start...")
		start = time.Now()
		computedPairs, randomMatrix, slidingRandomMatrixSum, tsProfiles, sketchDuration, searchDuration = singleParcorrSearch(allTimeSeries, i*basicWindowSize, randomMatrix, slidingRandomMatrixSum, tsProfiles)
		duration = time.Since(start)
		parcorrDurations = append(parcorrDurations, duration)
		parcorrSketchDurations = append(parcorrSketchDurations, sketchDuration)
		parcorrSearchDurations = append(parcorrSearchDurations, searchDuration)
		log.Println("ParCorr Finished. Costs ", duration)

		log.Println("Linear Search Start...")
		start = time.Now()
		gtPairs = parallelSingleLinearSearch(allTimeSeries, i*basicWindowSize)
		duration = time.Since(start)
		linearSearchDurations = append(linearSearchDurations, duration)
		log.Println("Linear Search Finished. Costs ", duration)
	...
	log.Println("Average Accuracy is:", sum/float64(len(accuracies)))

	var durationSum time.Duration

	durationSum = 0
	for _, duration := range parcorrSketchDurations {
		durationSum += duration
	}
	log.Println("Average ParCorr sketch time is:", durationSum/time.Duration(len(parcorrSketchDurations)))

	durationSum = 0
	for _, duration := range parcorrSearchDurations {
		durationSum += duration
	}
	log.Println("Average ParCorr search time is:", durationSum/time.Duration(len(parcorrSearchDurations)))

	durationSum = 0
	for _, duration := range parcorrDurations {
		durationSum += duration
	}
	log.Println("Average ParCorr total time is:", durationSum/time.Duration(len(parcorrDurations)))

	durationSum = 0
	for _, duration := range linearSearchDurations {
		durationSum += duration
	}
	log.Println("Average Linear Search total time is:", durationSum/time.Duration(len(linearSearchDurations)))
